...
import { getAccessorsByUnderscoredProperty } from '../utils';
...
enum MemberKind {
    ...
    protectedAccessoryInstanceField,
    privateInstanceField,
    privateAccessoryInstanceField,
    publicConstructor,
...
const allMemberKindNames = Lint.Utils.mapDefined(Object.keys(MemberKind), (key) => {
...
const optionsDescription = Lint.Utils.dedent`
    ...
    You can also omit the access modifier to refer to "public-", "protected-", and "private-" all at once; for example,
    "static-field".
    ...
    The '${OPTION_ALPHABETIZE}' option will enforce that members within the same category should be alphabetically
    sorted by name.`;
...
export class Rule extends Lint.Rules.AbstractRule {
    ...
    static FAILURE_STRING_ALPHABETIZE(prevName: string, curName: string) {
...
class MemberOrderingWalker extends Lint.AbstractWalker<IOptions> {
    private readonly fixes: [Lint.RuleFailure, Lint.Replacement][] = [];
     ...
     */
    private checkMembers(members: ts.NodeArray<Member>) { // tslint:disable-line: max-func-body-length
        let prevRank = -1;
...
function memberKindForMethodOrField(
    ...
    kind: 'Method' | 'Field' | 'FieldAccessory'
): MemberKind {
...
function getMemberKind(member: Member): MemberKind | undefined {
    // it was included additional levels with label "Accessory" which is property for accessor
    let accessLevel: Access = hasModifier(member.modifiers, ts.SyntaxKind.PrivateKeyword)
        ? 'private'
        : hasModifier(member.modifiers, ts.SyntaxKind.ProtectedKeyword)
            ? 'protected'
            : 'public';

    if ((member.kind === ts.SyntaxKind.PropertyDeclaration && belongToAccessor()) && accessLevel !== 'public') {
        accessLevel =  `${accessLevel}Accessory` as Access;
    }
    ...
    function belongToAccessor() {
        const accessors = getAccessorsByUnderscoredProperty(member as ts.PropertyDeclaration);

        return accessors.getter || accessors.setter;
    }
}
...
// extended accessory-levels for accessors
type Access = 'public' | 'protected' | 'private' | 'privateAccessory' | 'protectedAccessory' ;
...
function parseOptions(options: any[]): IOptions {
    ...
    const order = orderJson.map(
        (cat) =>
            typeof cat === 'string'
                ...
                : new MemberCategory(cat.name, new Set(Lint.Utils.flatMap(cat.kinds, memberKindFromName)))
    );
...
function convertFromOldStyleOptions(options: string[]): MemberCategoryJson[] {
    let categories: INameAndKinds[] = [{ name: 'member', kinds: allMemberKindNames }];
...
interface INameAndKinds {
    name: string;
    kinds: string[];
}
function splitOldStyleOptions(
    categories: INameAndKinds[],
    filter: (name: string) => boolean,
    a: string,
    b: string
): INameAndKinds[] {
    const newCategories: INameAndKinds[] = [];
...
function getNextSplitIndex(text: string, pos: number) {
    ...
    // tslint:disable: naming-convention number-literal-format
    ...
    // tslint:enable: naming-convention number-literal-format

    scan: while (newPos >= 0 && newPos < text.length) {
        const ch = text.charCodeAt(newPos);
        switch (ch) {
            case CharacterCodes.carriageReturn:
                if (text.charCodeAt(newPos + 1) === CharacterCodes.lineFeed) {
                    newPos++;
                }
            // falls through
            ...
            case CharacterCodes.slash:
                const nextChar = text.charCodeAt(newPos + 1);
                if (nextChar === CharacterCodes.slash || nextChar === CharacterCodes.asterisk) {
                    const isSingleLineComment = nextChar === CharacterCodes.slash;
                    newPos += 2; // tslint:disable-line: no-magic-numbers
                    if (isSingleLineComment) {
                        while (newPos < text.length) {
                            if (ts.isLineBreak(text.charCodeAt(newPos))) {
                        ...
                        while (newPos < text.length) {
                            if (
                                text.charCodeAt(newPos) === CharacterCodes.asterisk &&
                                text.charCodeAt(newPos + 1) === CharacterCodes.slash
                            ) {
                                newPos += 2; // tslint:disable-line: no-magic-numbers
                                continue scan;
                            }
