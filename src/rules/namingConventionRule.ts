...
enum Types {
    // tslint:disable:naming-convention no-magic-numbers
    default = -1,
    variable = 1,
    function = 1 << 1,
    parameter = 1 << 2,
    ...
    // tslint:enable:naming-convention no-magic-numbers
}
...
class NormalizedConfig {
    ...
    // tslint:disable-next-line:no-reserved-keywords
    matches(type: TypeSelector, modifiers: number, name: string): [boolean, boolean] {
...
class NameChecker {
    ...
    check(name: ts.Identifier, walker: Lint.AbstractWalker<any>) {
    ...
    private _checkPrefixes(identifier: string, name: ts.Identifier, prefixes: string[],
                           walker: Lint.AbstractWalker<any>): string {
    ...
    private _checkSuffixes(identifier: string, name: ts.Identifier, suffixes: string[],
                           walker: Lint.AbstractWalker<any>): string {
...
class IdentifierNameWalker extends Lint.AbstractWalker<NormalizedConfig[]> {
    ...
    private _usage: Map<ts.Identifier, utils.VariableInfo> | undefined;
    ...
    visitEnumDeclaration(node: ts.EnumDeclaration) {
        ...
        // treat enum members as public static readonly properties
    ...
    visitClassLikeDeclaration(node: ts.ClassLikeDeclaration) {
    ...
    visitParameterDeclaration(node: ts.ParameterDeclaration) {
        ...
        if (isNameIdentifier(node)) {
            ...
            this._checkDeclaration(
                ...
                // tslint:disable-next-line:no-non-null-assertion
                utils.isFunctionWithBody(node.parent!) && !parameterProperty && this._isUnused(node.name)
                    ? Modifiers.unused
                    : 0
            );
        } else {
    ...
    visitFunction(node: ts.FunctionDeclaration | ts.FunctionExpression) {
        if (node.name !== undefined) {
            this._checkDeclaration(<(ts.FunctionDeclaration | ts.FunctionDeclaration) & { name: ts.Identifier }> node,
    ...
    private _checkVariableDeclarationList(list: ts.VariableDeclarationList, modifiers: number) {
        ...
        if ((list.flags & ts.NodeFlags.Const) !== 0) {
            currentModifiers |= Modifiers.const;
        }
        utils.forEachDeclaredVariable(list, (declaration) => {
            let selector = TypeSelector.variable;
            if (declaration.kind === ts.SyntaxKind.BindingElement && !isEqualName(declaration.name,
        ...
        if (node.modifiers !== undefined) {
            if (type & Types.member) { // property, method, parameter property
                if (utils.hasModifier(node.modifiers, ts.SyntaxKind.PrivateKeyword)) {
                    currentModifiers |= Modifiers.private;
                } else if (utils.hasModifier(node.modifiers, ts.SyntaxKind.ProtectedKeyword)) {
                    currentModifiers |= Modifiers.protected;
                } else {
                    currentModifiers |= Modifiers.public;
                }
                ...
                if (utils.hasModifier(node.modifiers, ts.SyntaxKind.StaticKeyword)) {
                    currentModifiers |= Modifiers.static;
                }
            }
            ...
            if (utils.hasModifier(node.modifiers, ts.SyntaxKind.ExportKeyword)) {
                currentModifiers |= Modifiers.export;
            }
            if (utils.hasModifier(node.modifiers, ts.SyntaxKind.AbstractKeyword)) {
                currentModifiers |= Modifiers.abstract;
            }
        }

        if (type !== TypeSelector.property && type !== TypeSelector.method) {
            currentModifiers |= this._depth !== 0 ? Modifiers.local : Modifiers.global;
        }
...
function hasStrictCamelHumps(name: string, isUpper: boolean) {
    ...
    for (let i = 1; i < name.length; ++i) {
        ...
        if (currentIsUpper === isUppercaseChar(name[i])) {
            if (currentIsUpper) {
                return false;
            }
        } else {
